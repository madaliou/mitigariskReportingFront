...
<!--    </vx-card>-->
    ...
    <vs-table multiple v-model="selected"  ref="table" pagination   :max-items="itemsPerPage" search :data="recipients">
...
<!--          </div>-->
          ...
          <!--            Exporter-->
          <!--          </download-excel>-->
                    <div class="btn-add-new p-2 mb-6 mr-2 ml-2 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary" @click.stop="codingAll()">
                        <feather-icon icon="GridIcon" svgClasses="h-4 w-4" />
                        <span class="ml-2 text-base text-primary">Codifier</span>
                    </div>
            ...
            <div class="flex">
              <vx-tooltip text="Voir" color="primary">
                <feather-icon style="color: navy " icon="EyeIcon" svgClasses="w-5 h-5 hover:text-primary stroke-current" @click="activePrompt2 = true" v-on:click="openPlayerDialog(tr)" color="warring" type="border" class="ml-4" />
                          ...
                          <vx-tooltip text="Codifier un bien" color="warning">
                          <feather-icon  style="color: #FF9F43 " icon="GridIcon" svgClasses="w-5 h-5 hover:text-warning stroke-current" @click.stop="codingOne(tr.id)" class="ml-4" />
                          </vx-tooltip>
  ...
  watch: {
    selected (value) {
      ...
      for (let i = 0; i < value.length; i++) {
        this.properties.push(value[i].id)
      }
  ...
  methods: {
    ...
    deleteData (id) {
      this.$vs.dialog({
        ...
        accept: async () => {
          this.$vs.loading()
          this.$http.delete(`notYetCodifiedProperties/${id}/`)
            .then(response => {
    ...
    getAllRecipients () {
      ...
      this.$http.get('notYetCodifiedProperties/')
        .then((response) => {
          this.verif_filter = false
          this.recipients = response.data.results
          this.repNext = `${str}notYetCodifiedProperties/?limit=100&offset=100`
    ...
    codingAll () {
      if (this.properties.length !== 0) {
        this.$vs.dialog({
          ...
          color: 'warning',
          title: 'Confirmation',
          text: 'Confirmer la codification.',
          acceptText: 'Confirmer',
          cancelText: 'Annuler',
          accept: async () => {
            this.$vs.loading()
            this.$http.post('codifyProperties/', { properties:this.properties})
              .then(response => {
                window.getPrendTaCom.success('Bien(s) codifié(s) avec succès.', response)
                this.recipients.splice(0, this.recipients.length)
                this.properties.splice(0, this.properties.length)
                this.getAllRecipients()
              ...
              .catch(() => {
                window.getPrendTaCom.error({ message: 'La codification du bien a échoué.' })
              })
          }
        })
      } else {
        this.$vs.notify({
          time:4000,
          title: 'ATTENTION',
          text: 'Selectionnez le(s) bien(s) à codifier.',
          color: 'warning',
          position:'top-center'
        })
        this.$vs.loading.close()
    ...
    codingOne (id) {
      ...
      this.$vs.dialog({
        ...
        accept: async () => {
          this.$vs.loading()
          this.$http.post('codifyProperties/', {properties:this.propertie})
            .then(response => {
              window.getPrendTaCom.success('Bien codifié avec succès.', response)
              this.getAllRecipients()
            })
            .catch(() => {
              window.getPrendTaCom.error({ message: 'La codification du bien a échoué.' })
