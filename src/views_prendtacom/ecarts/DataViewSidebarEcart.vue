  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <component :is="scrollbarTag" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      <div class="p-6">
        ...
        <vs-input name="observation" v-model="input.observation" class="w-full" />
  ...
  methods: {
    ...
    async submitData () {
      ...
      this.$http[methods](url, this.dataSave)
        .then(() => {
          if (window.getProspect.verif_filter) {
            window.getProspect.getAllEcarts_fiter()
          } else {
            window.getProspect.Biens_perdus()
          }
          this.$vs.notify({
            title: message.titre,
